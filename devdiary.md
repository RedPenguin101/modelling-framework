...
## 10th June
### Outputs
As you build more of a model, the dependencies of any given row grow in number, and predicting what the impact of your model changes on the output becomes harder.

* Has the change I've made invalidated any invariants I've set
* What impact has the change had on my (income statement/cashflow totals) [this presumably needs to be flexible enough to handle other things.]

This needs to be tracked over several runs.

This is _not_ the same has having multiple scenarios.

In the [Gridlines model](https://academy.gridlines.com/courses/434503/lectures/14835264), Kenny frequently goes to his 'outputs' sheet to see what has changed. He stores the outputs (totals of IS/CFS) from the previous run in a column (which can be named), change inputs/the model, and see the impact on the outputs, include the delta.

![Outputs](./docs/Outputs.png)

There are two types of outputs:

1. Sheet outputs: Taking the totals/summaries of every row in a sheet, and showing that.
2. Calculated output: A function applied to full rows of data e.g. IRR calcs.

Is there a better way to show it than like the spreadsheet? Maybe I can show 'only changes'. i.e. hide anything where nothing has changed.
...
* Keyboard navigation
* Can navigate between sheets/period ranges without running code
* Can flip between outputs and results
* hover for trace (and click to navigate)
* select comparison on outputs
* Dynamic charting
* Warnings click
* Comments? Maybe better to keep in code
