...
(defn- draw-axis-lines2
  ...
  (def debug {:v-size v-size
              :v-zero v-zero
              :zero-scaled (* v-zero v-size)
              :old-v (int (* 0.9 v-size))
              :new-v (int (* v-size (- 1 v-zero)))
              :vert-ax [(int (* 0.1 h-size)) (int (* 0.9 h-size))]})
  (let [margin 0.1
...
(defn series-lines [series]
  (let [canvas   (c2d/canvas 1000 1000)
        ...
        z-point  (max 0 (/ (- (apply min c-series)) y-range))
        x-vals   (map #(+ 100 %) (calc-and-scale (range 0 (apply max (map count series))) 800))]
    (println "y-scale" y-scale)
    (println "y-range" y-range)
    (println "z-point" z-point)
    (println "x-vals" x-vals)
