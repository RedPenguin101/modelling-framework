...
(def time-calcs
  (merge
   #:time.period
    {:start-date              '(if (true? [:flags/first-model-column])
...
(def flags
  #:flags
   {:first-model-column       '(= 1 [:time/model-column-number])
...
(def revenue
  #:revenue
   {:compound-degradation         '(when-flag [:flags/operating-period]
